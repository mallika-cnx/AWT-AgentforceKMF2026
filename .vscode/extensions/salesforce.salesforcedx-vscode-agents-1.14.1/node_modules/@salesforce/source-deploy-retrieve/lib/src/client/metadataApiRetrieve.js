...
class RetrieveResult {
     ...
     */
    constructor(response, components, localComponents, partialDeleteFileResponses = [], registry) {
        this.response = response;
        this.components = components;
        this.partialDeleteFileResponses = partialDeleteFileResponses;
        this.localComponents = new componentSet_1.ComponentSet(localComponents?.getSourceComponents(), registry);
    }
    getFileResponses() {
...
class MetadataApiRetrieve extends metadataTransfer_1.MetadataTransfer {
    ...
    constructor(options) {
        ...
        if (this.mdapiTempDir) {
            this.mdapiTempDir = (0, node_path_1.join)(this.mdapiTempDir, `${new Date().toISOString().replace(/[<>:"\\|?*]/g, '_')}_retrieve`);
        }
    }
    /**
    ...
    async checkStatus() {
        if (!this.id) {
            throw new sfError_1.SfError(messages.getMessage('error_no_job_id', ['retrieve']), 'MissingJobIdError');
        }
    ...
    async post(result) {
        ...
        const retrieveResult = new RetrieveResult(result, componentSet, this.components, partialDeleteFileResponses, this.options.registry);
        if (!isMdapiRetrieve && !this.options.suppressEvents) {
            ...
            await lifecycle_1.Lifecycle.getInstance().emit('scopedPostRetrieve', {
                retrieveResult,
                orgId: this.orgId,
            });
        }
    ...
    async pre() {
        ...
        if (this.components?.size === 0 && !packageNames?.length) {
            throw new sfError_1.SfError(messages.getMessage('error_no_components_to_retrieve'), 'MetadataApiRetrieveError');
        }
        const connection = await this.getConnection();
        ...
        if (this.options.components && !this.options.suppressEvents) {
            await lifecycle_1.Lifecycle.getInstance().emit('scopedPreRetrieve', {
        ...
        if (manifestVersion) {
            ...
            await lifecycle_1.Lifecycle.getInstance().emit('apiVersionRetrieve', { manifestVersion, apiVersion });
        }
