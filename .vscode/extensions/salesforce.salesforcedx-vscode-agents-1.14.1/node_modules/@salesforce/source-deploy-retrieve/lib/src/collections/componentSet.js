...
class ComponentSet extends lazyCollection_1.LazyCollection {
    ...
    static async fromManifest(input) {
        ...
        if (hasBotVersions) {
            const botVersionFilters = processBotVersionsInManifest(manifest.components, options.registry, result.logger, addComponent);
    ...
    getComponentFilenamesByNameAndType({ fullName, type }) {
        ...
        if (parentType && fullName.includes('.')) {
            ...
            const parentKey = (0, exports.simpleKey)({ fullName: parentFullName, type: parentType });
...
const collectFilenames = (components) => {
    const output = new Set();
    for (const component of components) {
        if (component instanceof sourceComponent_1.SourceComponent) {
            [...component.walkContent(), component.content, component.xml].filter(ts_types_1.isString).map((f) => output.add(f));
        }
    }
 ...
 */
function processBotVersionsInManifest(components, registry, logger, addComponent) {
    const botVersionFilters = [];
    for (const component of components) {
        if (component.type.name === 'BotVersion') {
            // Parse BotVersion fullName format: BotName.vN (e.g., Local_Info_Agent.v4)
            const { botName, versionFilter } = (0, agentResolver_1.parseBotVersionFullName)(component.fullName);
