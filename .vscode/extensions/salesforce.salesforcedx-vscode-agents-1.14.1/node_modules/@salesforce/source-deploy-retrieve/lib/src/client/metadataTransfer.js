...
exports.calculateErrorRetryLimit = exports.calculatePollingFrequency = exports.normalizePollingInputs = exports.MetadataTransfer = void 0;
/*
...
class MetadataTransfer {
    ...
    async pollStatus(frequencyOrOptions, timeout) {
        ...
        this.errorRetryLimit = (0, exports.calculateErrorRetryLimit)(this.logger);
        ...
        try {
            ...
            if (this.errorRetryLimitExceeded !== undefined) {
                const errorData = this.errorRetryLimitExceeded;
                throw new core_1.SfError(messages.getMessage('error_retry_limit_exceeded', [errorData.count, errorData.error.message]), 'MetadataTransferError');
            }
            ...
            if (err instanceof core_1.SfError && err.message.includes('consecutive retryable errors')) {
        ...
        if (this.canceled) {
            ...
            try {
                ...
                if (this.isRetryableError(e)) {
                    ...
                    this.logger.debug(`Network error on request (retry ${this.consecutiveErrorRetries}/${this.errorRetryLimit})`, e);
 ...
 */
const calculateErrorRetryLimit = (logger) => {
    const envRetryLimit = process.env.SF_METADATA_POLL_ERROR_RETRY_LIMIT;
    if (envRetryLimit) {
        const parsedLimit = parseInt(envRetryLimit, 10);
        if (!isNaN(parsedLimit) && parsedLimit > 0) {
            logger.debug(`Using error retry limit from SF_METADATA_POLL_ERROR_RETRY_LIMIT environment variable: ${parsedLimit}`);
            return parsedLimit;
        }
    }
    return 1000;
};
exports.calculateErrorRetryLimit = calculateErrorRetryLimit;
