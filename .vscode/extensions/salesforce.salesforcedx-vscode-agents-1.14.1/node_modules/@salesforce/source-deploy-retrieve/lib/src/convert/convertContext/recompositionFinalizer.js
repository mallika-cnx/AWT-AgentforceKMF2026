...
exports.RecompositionFinalizer = void 0;
/*
...
const recompose = (cache) => async (stateValue) => {
    ...
    const childXmls = await Promise.all(childComponents.filter(ensureMetadataComponentWithParent).map(async (child) => ({
        cmp: child,
...
const getStartingXml = (cache) => async (parent) => parent.type.strategies?.recomposition === 'startEmpty'
    ? {}
    : (0, decomposed_1.unwrapAndOmitNS)(parent.type.name)(await getXmlFromCache(cache)(parent)) ?? {};
/** throw if the parent component isn't in the state entry */
const ensureStateValueWithParent = (stateValue) => {
    if (stateValue.component) {
        return true;
    }
    throw new Error(`The parent component is missing from the recomposition state entry.  ${stateValue.children
